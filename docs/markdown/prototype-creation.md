...
```
### Чтобы была возможность покупки шаттла на вашем гриде то должен быть компонент<span class="hover-container"><strong>`StationMember`</strong><span class="hover-text"> StationMember - Компонент который подключается к BecomeStation для совместной работы</span></span>он выдается автоматически после спавна грида через прототип
